...
function RoutePlannerComponent() {
  ...
  }, [origin, destination, travelMode, isLoaded, calculateRoute]);
  ...
  return (
    <Grid container spacing={2} sx={{ p: {xs: 1, sm: 2}, flexGrow: 1, overflow: 'hidden' }}>
      <Grid item xs={12} md={3} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Paper elevation={3} sx={{ p: 2, display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto' }}>
          ...
          <Button
            variant="contained"
            color="primary"
            onClick={calculateRoute} // El botón "Calcular Ruta" sigue llamando directamente
            fullWidth
            sx={{ mt: 2, py: 1.2 }}
...
function Pages15() {
  return (
    <ThemeProvider theme={darkTheme}>
      <CssBaseline />
      <Box sx={{display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)', p: {xs:1, md:2}, bgcolor: 'background.default' }}>
        <Typography variant="h4" component="h1" align="center" color="primary.main" gutterBottom sx={{fontWeight: 'bold', mb:2, flexShrink: 0}}>
          Actividad 15: Rutas Dinámicas y Autocompletado
        </Typography>
